# 🗄️ Sprint 0 — Entity Relationship Diagram (ERD)
### Hệ thống Quản lý Bán hàng Quán Cà phê / Trà sữa
**Phiên bản:** 1.0 | **Ngày:** 11/06/2026
...
erDiagram
    ...
    Orders }o--o| Orders : "parent sub-order"
...
| SubOrderIndex | int | 1 = đơn gốc, 2+ = gọi thêm |
| ParentOrderId | int FK | Null nếu đơn gốc |
| Status | enum | `Pending` → `Confirmed` → `Preparing` → `Completed` → `Closed` \| `Cancelled` |
| PaymentStatus | enum | `Unpaid` → `Paid` |
| PaymentMethod | enum | `Cash` `Transfer` `Mixed` |

> **Điều kiện đóng đơn:** `Status = Completed` AND `PaymentStatus = Paid` → Tự động `Closed`
...
```
POS (DineIn / TakeAway):
  [Pending] ──auto──▶ [Confirmed] ──▶ [Preparing] ──▶ [Completed] ──▶ [Closed✅]
                                            │                │
                                      Bar Done +       Paid ✓
                                     Pastry Done

Online:
  [Pending] ──CSR confirm──▶ [Confirmed] ──▶ [Preparing] ──▶ [Completed] ──▶ [Closed✅]
...
| Quy tắc | Giá trị |
|---------|----------|
| Soft Delete | Dùng `IsActive = false` thay vì DELETE |
| Giá lưu lịch sử | `UnitPrice`, `ToppingPrice` lưu giá **tại thời điểm đặt** |
| Audit Discount | `OrderDiscounts.ApprovedByStaffId` ghi nhận SL/Owner xác nhận |
...
| InventoryCheck Status | `PendingApproval` → `Approved` — SL tạo, Owner duyệt mới overwrite kho |
| In ấn | Không có trong phạm vi hiện tại |
| Huỷ món Online | CSR xóa `OrderItem` khi đơn ở `Pending` — Recalculate tổng đơn |
